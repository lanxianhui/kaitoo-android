...
import com.timesoft.kaitoo.wsclient.main.bo.UserBO;

public class UserCoreAuthentificationTask extends
		CommonWebService<SoapObject, ResponseCommon> {

	private static final String TAG = "UserCoreAuthentificationTask";

	protected static final String WS_METHOD_NAME = "userCoreAuthentification";
	...
	private ResponseCommon parseSOAPResponse(SoapObject response) {
		...
		if (response != null
			...
			SoapObject userForecastNode = (SoapObject) response
			...
			if (userForecastNode != null) {
				...
				Long id = Long.parseLong(userForecastNode
						.getPrimitivePropertySafelyAsString("id"));
				String email = userForecastNode
						.getPrimitivePropertySafelyAsString("email");
				String password = userForecastNode
						.getPrimitivePropertySafelyAsString("password");
				UserBO userForecastResult = new UserBO(id, email, password);

				Log.i(TAG, " --> id: " + id + ", email: " + email
						+ ", password: " + password + ".");
